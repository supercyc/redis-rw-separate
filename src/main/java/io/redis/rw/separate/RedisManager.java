...
public class RedisManager {
		...
		public void run() {
			...
			while (true) {
				...
				try {
					...
					ret = curr.blpop(timeout, listName.getBytes("utf-8"))
							.get(1);// 取出一个消息，没有则等待
				} catch (Exception e) {
	...
	public String pop(final String listName, final boolean blockingWhenEmpty,
			final int timeout) {
	...
	public byte[] popByte(String listName, boolean blockingWhenEmpty,
		...
		if (blockingWhenEmpty) {
			popThreads.putIfAbsent(listName + timeout, new BlockingPoPThread(
					timeout, listName));
	...
	public void subscribe(final String interestingName,
			final SubscribeAdapter callback) {
		initOP(OP.READ);
...
class RedisManagerProxy implements MethodInterceptor {
	...
	public RedisManagerProxy(String ips) {
		...
		try {
			...
			for (int i = 0; i < ip.length; i++) {
				...
				if (ipinfo.length == 2) {
					HAJedisInfo jedisInfo = new HAJedisInfo(ipinfo[0],
							Integer.valueOf(ipinfo[1]));
					if (i == 0)
						jedisInfo.setIsmaster(true);
					list.add(jedisInfo);
				}
	...
	private class HAJedisPool {
		...
		public HAJedisPool(Config config, String ip, int port, int timeout) {
			...
			this.slavePool = new GenericObjectPool(new HAJedisSlaveFactory(
					HAJedisInfo, hafc), config);
		...
		private class HAJedisSlaveFactory extends BasePoolableObjectFactory
				implements ChangeMasterListener {
			...
			public Object makeObject() throws Exception {
				...
				HAJedis jedis = new HAJedis(currentSlave.getIp(),
						currentSlave.getPort(), currentSlave.getTimeout());
				return jedis;
			}
				...
				try {
					...
					for (HAJedisInfo ha : HAJedisList) {
						...
						if (currentSlave == null) {
							if (!ha.isIsmaster()
									&& !masterFactory.currentMaster.equals(
											ha.getIp(), ha.getPort())
									&& checkIsAlive(ha.getIp(), ha.getPort())) {
								currentSlave = new HAJedis(ha.getIp(),
										ha.getPort(), ha.getTimeout());
								finded = true;
								break;
							}
						}
						// 后继选择时
						if (!masterFactory.currentMaster.equals(ha.getIp(),
								ha.getPort())
								&& checkIsAlive(ha.getIp(), ha.getPort())) {
		...
		private class HAJedisFactory extends BasePoolableObjectFactory {
			...
			public HAJedisFactory(List<HAJedisInfo> HAJedisInfo) {
				...
				heart.start();
				// 有问题这个schedule.scheduleWithFixedDelay(new HeartBeat(),0,1,
				// TimeUnit.SECONDS);
			}

			public synchronized Object makeObject() throws Exception {
				checkConnection();
				HAJedis jedis = new HAJedis(currentActiveHAJedis.getIp(),
						currentActiveHAJedis.getPort(),
						currentActiveHAJedis.getTimeout());
				return jedis;
			}
			...
			private synchronized void findOneSavleAsMaster() {
				for (HAJedisInfo ha : HAJedisList) {
					String currIP = currentActiveHAJedis.getIp() + ":"
							+ currentActiveHAJedis.getPort();
					String haIP = ha.getIp() + ":" + ha.getPort();
					if (!ha.isIsmaster() && !currIP.equals(haIP)) {
						HAJedis jedis = null;
						try {
							jedis = new HAJedis(ha.getIp(), ha.getPort(),
									ha.getTimeout());
			...
			private synchronized void findBaseMaster() {
				...
				for (HAJedisInfo ha : HAJedisList) {
					if (ha.isIsmaster()) {
						baseMaster = new HAJedis(ha.getIp(), ha.getPort(),
								ha.getTimeout());
			...
			private synchronized void changeMasterTo(String toip, int toport) {
				...
				for (HAJedisInfo ha : HAJedisList) {
					if (toip.equals(ha.getIp()) && (toport == ha.getPort())) {
						if (!baseMaster.equals(currentMaster))
							destroyObject(currentMaster);
						this.currentMaster = new HAJedis(ha.getIp(),
								ha.getPort(), ha.getTimeout());
				...
				for (ChangeMasterListener cm : obserers) {
					cm.changeMaster(currentActiveHAJedis.getIp(),
							currentActiveHAJedis.getPort());
				}
				logger.warn("switchToMaster-->" + currentActiveHAJedis.getIp()
						+ ":" + currentActiveHAJedis.getPort());
			}
